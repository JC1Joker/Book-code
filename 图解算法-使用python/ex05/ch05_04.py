    ...
    while True:
        if SearchKey==data[RootNode]:
            ...
            if index==2:
                return MAX # 没有找到
            if SearchKey<data[RootNode]:
                RootNode=RootNode-diff2  # 左子树的新斐波那契数
                temp=diff1
                diff1=diff2      # 前一个斐波那契数
                diff2=temp-diff2 # 前两个斐波那契数
                index=index-1
            else:
                if index==3:
                    return MAX
                RootNode=RootNode+diff2  # 右子树的新斐波那契数
                diff1=diff1-diff2  # 前一个斐波那契数
                diff2=diff2-diff1  # 前两个斐波那契数
                index=index-2
...
while True:
    ...
    if val==-1: # 输入值为-1就跳离循环
        break
    RootNode=fib_search(data,val)# 使用斐波那契查找法查找数据
    if RootNode==MAX:
        print('##### 没有找到[%3d] #####' %val)
    else:
